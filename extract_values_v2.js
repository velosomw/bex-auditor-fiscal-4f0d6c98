...
async function extract() {
    ...
    worksheet.eachRow((row, rowNumber) => {
        ...
        data.forEach(m => {
            ...
            if (account === '2.3' || desc === 'PATRIMÔNIO LÍQUIDO') m.pl = val;
            // Receita Líquida - Geralmente no final ou DRE
            if (desc && (desc.includes('RECEITA LÍQUIDA') || desc.includes('RECEITA OPERACIONAL LÍQUIDA'))) {
                m.rl = val;
            }
        });
    });

    // Se PT não foi encontrado, talvez esteja como "PASSIVO E PATRIMONIO LIQUIDO"
    // Vamos fazer uma segunda passada se necessário ou ajustar os filtros.
