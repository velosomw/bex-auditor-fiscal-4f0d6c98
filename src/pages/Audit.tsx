          ...
          <div>
            ...
            <div className="space-y-3 report-keep-together break-inside-avoid">
              {[
                ...
                <div key={item.title} className="p-3 rounded-lg bg-muted/20 border border-border/30 report-card-keep-together mb-2 break-inside-avoid" style={{ pageBreakInside: 'avoid', breakInside: 'avoid-page' }}>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
              {([
                ...
                { label: "Obrigações Tributárias (LP)", value: snapshot?.residual?.tax?.noncurrent_obligations?.value || snapshot?.facts?.tax_noncurrent || 0, available: true, scope: "Grupo 2.2.3 (Certified Binding)" },
          ...
          <div className="px-4">
            ...
            <div className="relative h-12 rounded-full overflow-hidden bg-gradient-to-r from-red-500 via-orange-400 via-yellow-500 via-blue-400 to-emerald-500">
              {kanitzResults.filter(r => r.kanitzAplicavel).map(r => {
                ...
                return (
                  <div key={r.year} className="absolute top-0 bottom-0 w-1 bg-foreground rounded-full shadow-lg" style={{ left: `${pos}%`, transform: "translateX(-50%)" }} title={`${r.year}: FI = ${fiFmt(r.fi, r.kanitzAplicavel)}`}>
