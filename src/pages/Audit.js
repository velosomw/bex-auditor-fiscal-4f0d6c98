...
import { Upload, FileText, CheckCircle2, ArrowRight, ArrowLeft, Shield, MessageCircle, Send, AlertTriangle, Download, Printer, Calculator, TrendingUp, BarChart3, PieChart, Activity, Target, Scale, Layers, Building2, Loader2, FileSpreadsheet, Landmark, AlertOctagon, Search, ChevronDown, Settings, FileSearch, BookOpen, Database, Info, ChevronLeft, ChevronRight, Clock, FileCheck } from "lucide-react";
...
import { computeIndicatorsForRow } from "@/services/indicatorsEngine";
import PlatformLayout from "@/components/PlatformLayout";
import { useUrlScrollSync } from "@/hooks/useUrlScrollSync";
import { parseFile, parseMultipleFiles, analyzeFinancialData, runAuditPipeline, streamAuditChat, inferRefByCode } from "@/services/auditAIService";
...
import { buildBSDados, exportBSDadosToCSV, mesKeyToLabel } from "@/services/bsDadosBuilder";
import { DedupPresetForm } from "@/components/audit/DedupPresetForm";
import { toast } from "@/hooks/use-toast";
import { saveAuditBatch, saveGeneratedReport } from "@/services/auditHistoryService";
import { canGenerateForCompany, getAuditMonthsCap } from "@/services/reportLimitsService";
import { getFileFormat as getFormat } from "@/services/auditAIService";
import { mergeMultiMonth, pickMonths, detectMonthRangeFromFilename, extractColumnMonths, reconcileMonthsWithFilename } from "@/services/auditMonthDetector";
import { readWorkbook } from "@/lib/excelReader";
import { MonthsConfirmDialog } from "@/components/audit/MonthsConfirmDialog";
import { buildCertifiedFinancialSnapshot } from "@/services/canonicalFinancialSnapshotService";
...
const fiFmt = (fi, aplicavel = true) => aplicavel && typeof fi === "number" && Number.isFinite(fi) ? fi.toFixed(4) : "N/A";
const fmt = (n) => {
    if (n == null || isNaN(n))
        return "N/A";
...
const printReport = (containerId, reportTitle) => {
...
const exportPdf = async (containerId, reportTitle) => {
    ...
    clone.querySelectorAll('.report-pages-container').forEach(n => {
        n.style.padding = '0';
    ...
    const pages = Array.from(clone.querySelectorAll('.report-a4-page, .report-a4-cover'));
    pages.forEach((p, idx) => {
...
const exportDocx = (containerId, reportTitle) => {
...
const StepTimeline = ({ currentStep }) => (<div className="w-full mb-8">
    <div className="flex items-center justify-between relative">
      {/* Connecting line */}
      <div className="absolute top-5 left-0 right-0 h-[2px] bg-border z-0"/>
      <div className="absolute top-5 left-0 h-[2px] z-[1] transition-all duration-700" style={{
      ...
      {timelineSteps.map((step) => {
        ...
        return (<div key={step.id} className="flex flex-col items-center z-10 relative" style={{ flex: 1 }}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${isComplete
              ...
              {isComplete ? (<CheckCircle2 className="w-5 h-5"/>) : (<Icon className={`w-4 h-4 ${isActive && step.icon === Loader2 ? "animate-spin" : ""}`}/>)}
            </div>
            <span className={`text-[11px] mt-2 font-medium text-center leading-tight ${isActive ? "text-[hsl(258,90%,66%)]" : isComplete ? "text-foreground" : "text-muted-foreground"}`}>
              {step.label}
            </span>
          </div>);
    })}
...
/* ══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
const UploadPhase = ({ onProcess, onFilesReady, onMesesReady, dedupConfig, onDedupChange, onDepthChange, onForceReprocess }) => {
    ...
    const [depth, setDepth] = useState("tecnico");
    useEffect(() => { onDepthChange?.(depth); }, [depth, onDepthChange]);
    const [purpose, setPurpose] = useState("externa");
    const [rawFiles, setRawFiles] = useState([]);
    // mes atribuído por documento: { docId: "2024-03" }
    const [fileMeses, setFileMeses] = useState({});
    const [fileIsYtd, setFileIsYtd] = useState({});
    ...
    const detectMesFromName = useCallback((name) => {
        const MES_NAMES = {
    ...
    const handleFiles = (fileList) => {
        ...
        newDocs.forEach(async (doc, i) => {
            const file = filesArr[i];
            if (!/\.(xlsx|xls|xlsm|xlsb|xltx|xltm)$/i.test(file.name))
                return;
            ...
            try {
                ...
                setFilePreview(prev => ({ ...prev, [doc.id]: { loading: false, months: [], error: String(err?.message || err) } }));
            }
        });
    };
    const removeFile = (id) => {
    ...
    const handleContinue = () => {
        ...
        const entries = state.config.files.map(f => ({
            fileName: f.fileName,
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          {hasFiles ? (<div className="space-y-3">
              {state.config.files.map(f => {
                const mes = fileMeses[f.id] || "";
                const needsMonth = !mes;
                return (<div key={f.id} className={`relative border-2 border-dashed rounded-2xl p-5 bg-emerald-50/30 ${needsMonth ? "border-red-400/70" : "border-emerald-400/50"}`}>
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-emerald-500/10 flex items-center justify-center shrink-0">
                        {(/\.(pdf)$/i).test(f.fileName) ? (<FileText className="w-6 h-6 text-emerald-600"/>) : (/\.(docx?|txt|rtf)$/i).test(f.fileName) ? (<FileText className="w-6 h-6 text-emerald-600"/>) : (/\.(json|xml|ofx|sped)$/i).test(f.fileName) ? (<FileSearch className="w-6 h-6 text-emerald-600"/>) : (<FileSpreadsheet className="w-6 h-6 text-emerald-600"/>)}
                      </div>
                    ...
                    {(() => {
                        ...
                        return (<div className={`mt-3 p-3 rounded-lg border ${needsMonth ? "border-red-400/60 bg-red-50/60" : "border-emerald-400/40 bg-emerald-50/40"}`}>
                          ...
                          <div className={`grid ${isAuto ? "grid-cols-1" : "grid-cols-[1fr_110px]"} gap-2`}>
                            <Select value={selectedMonth} onValueChange={setMonth}>
                              ...
                              <SelectContent className="max-h-72">
                                {monthOptions.map(opt => (<SelectItem key={opt.value} value={opt.value} className="text-xs">{opt.label}</SelectItem>))}
                              </SelectContent>
                            </Select>
                            {!isAuto && (<Select value={String(selectedYear)} onValueChange={(v) => setYear(Number(v))}>
                          ...
                          {isAuto && (() => {
                                ...
                                if (preview?.loading) {
                                    return (<div className="mt-2 text-[10px] text-muted-foreground bg-muted/40 border border-border rounded px-2 py-1.5">
                                  ⏳ Lendo cabeçalhos da planilha…
                                </div>);
                                }
                                if (detected.length === 0) {
                                    return (<div className="mt-2 text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1.5">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {detected.map(m => (<Badge key={m.key} variant="outline" className="text-[10px] bg-emerald-500/10 text-emerald-700 border-emerald-500/30">
                                      {m.label}
                                    </Badge>))}
                                </div>
                              </div>);
                    ...
                    <label className="mt-2 flex items-center gap-2 text-[11px] text-foreground cursor-pointer select-none px-1">
                      <input type="checkbox" checked={!!fileIsYtd[f.id]} onChange={(e) => setFileIsYtd(prev => ({ ...prev, [f.id]: e.target.checked }))} className="h-3.5 w-3.5 accent-emerald-600"/>
              ...
              {state.config.files.length < 3 ? (<button onClick={() => document.getElementById("file-input")?.click()} className="w-full py-2 text-xs text-muted-foreground hover:text-foreground border border-dashed border-border rounded-xl hover:bg-muted/30 transition-colors">
                  + Adicionar outro documento ({state.config.files.length}/3)
                </button>) : (<div className="w-full py-2 px-3 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-xl text-center">
        ...
        <div className="space-y-6">
        ...
        ].map(opt => (<button key={opt.id} onClick={() => setDepth(opt.id)} className={`w-full text-left p-4 rounded-xl border-2 transition-all ${depth === opt.id ? "border-[hsl(258,90%,66%)] bg-[hsl(258,90%,66%)]/5" : "border-border hover:border-[hsl(258,90%,66%)]/30 hover:bg-muted/20"}`}>
                    <div className="flex items-center justify-between">
                     <div>
                       <div className="flex items-center gap-2">
                         <p className="text-sm font-semibold text-foreground">{opt.title}</p>
                         {depth === opt.id && (<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${opt.id === "executivo" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                             {opt.id === "executivo" ? "Versão Gratuita" : "Versão Paga"}
                           </span>)}
                       </div>
                       <p className="text-xs text-muted-foreground mt-0.5">{opt.desc}</p>
                     </div>
                     {depth === opt.id && (<div className="w-5 h-5 rounded-full border-2 border-[hsl(258,90%,66%)] flex items-center justify-center shrink-0">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {purposes.map(p => (<button key={p.id} onClick={() => setPurpose(p.id)} className={`px-4 py-2.5 rounded-full text-xs font-medium border transition-all ${purpose === p.id ? "bg-[hsl(258,90%,66%)] text-white border-[hsl(258,90%,66%)]" : "bg-white border-border text-foreground hover:border-[hsl(258,90%,66%)]/40"}`}>{p.label}</button>))}
            </div>
          </div>
        </div>
      </div>
      ...
      {false && (<div className="max-w-3xl mx-auto pt-2">
          <DedupPresetForm value={dedupConfig} onChange={onDedupChange} disabled={isFreeTier} lockedMessage="Disponível apenas em planos pagos."/>
        </div>)}

      <div className="flex flex-col items-center pt-2 gap-2">
        {hasFiles && missingMeses.length > 0 && (<p className="text-xs text-red-600 font-medium flex items-center gap-1.5">
            <AlertTriangle className="w-3.5 h-3.5"/>
            Atribua o mês de referência em {missingMeses.length} documento(s) destacado(s) em vermelho.
          </p>)}
        <Button onClick={() => { onForceReprocess?.(false); handleContinue(); }} disabled={!canContinue} className="bg-[hsl(258,90%,66%)] hover:bg-[hsl(258,90%,56%)] text-white gap-2 h-12 px-10 text-sm font-semibold rounded-xl shadow-lg shadow-[hsl(258,90%,66%)]/20">
          Fazer Auditoria <ArrowRight className="w-5 h-5"/>
        </Button>
        {onForceReprocess && (<button type="button" onClick={() => { onForceReprocess(true); handleContinue(); }} disabled={!canContinue} title="Ignora o cache de dedup e reprocessa os balancetes com a versão mais recente do parser." className="text-xs text-muted-foreground hover:text-foreground underline underline-offset-4 disabled:opacity-50 disabled:no-underline">
...
const ProcessingPhase = ({ onComplete, files, onAnalysisReady, dedupConfig, preParsed, companyId, balanceteEntries, forceReprocess }) => {
    ...
    const [processingRunId, setProcessingRunId] = useState(""); // MD-CUTOVER-001 §6
    ...
    const nominalTotalSec = useMemo(() => Math.round(processingSteps.reduce((acc, s) => acc + (s.duration || 0), 0) / 1000), []);
        ...
        const runRealAnalysis = async () => {
            try {
                ...
                if (parsedData && (parsedData.balanco.length > 0 || parsedData.dre.length > 0)) {
                    try {
                        pipelineResult = await runAuditPipeline(parsedData, files[0]?.name || "balancete", companyId ?? undefined, undefined, dedupConfig, (ev) => {
                            if (ev.progress)
                                setPipelineProgress(ev.progress);
                            else if (ev.status)
                                setPipelineProgress(`Status: ${ev.status}`);
                        }, forceReprocess);
                        if (pipelineResult) {
                            setPipelineProgress(null);
                            console.log(`Pipeline IA — qualidade ${(pipelineResult.scores.quality * 100).toFixed(1)}% | mapeadas ${pipelineResult.normalized.filter(r => r.matched).length}/${pipelineResult.normalized.length} | few-shot ${pipelineResult.few_shot_examples.length}`);
                        }
                    }
                    catch (e) {
                    ...
                    try {
                        ...
                        if (meses.length === 0) {
                            ...
                            const ytdByMes = new Map();
                            for (const e of (balanceteEntries || [])) {
                                if (e.isYtd && e.mesReferencia && validKey(e.mesReferencia))
                                    ytdByMes.set(e.mesReferencia, true);
                            }
                            const balancetes = meses.map(mes => {
                                const linhas = allRows.map(r => {
                                    ...
                                    return {
                                        ...
                                        ref1: r.ref1 ?? r.refCapital ?? inferRefByCode(r.conta),
                                        saldo: Number(v) || 0,
                                    };
                            ...
                            if (balancetes.length > 0 && balancetes.some(b => b.linhas.length > 0)) {
                                ...
                                console.log(`BS & Dados (server) — ${persistResp.summary.meses} meses | ${persistResp.summary.total_linhas} linhas | persistido=${persistResp.persisted ?? false} | audit_id=${persistResp.audit_id ?? "—"}`);
                                // FIX #4: captura ground-truth para injetar como FATOS FIXOS na IA.
                                if (Array.isArray(persistResp.bsDados) && persistResp.bsDados.length > 0) {
                                    deterministicFacts = {
                                        bsDados: persistResp.bsDados,
                                        indicadores: persistResp.indicadores ?? [],
                                        kanitz: persistResp.kanitz ?? [],
                                        insights: persistResp.insights ?? null,
                                    };
                                }
                ...
                if (deterministicFacts?.insights) {
                    analysis.insightsDeterministicos = deterministicFacts.insights;
                }
    ...
    return (<div className="space-y-8">
      ...
      <div className="max-w-xl mx-auto space-y-8 py-8">
        ...
        <div className="rounded-2xl border border-[hsl(258,90%,66%)]/25 bg-gradient-to-br from-[hsl(258,90%,66%)]/8 to-transparent p-5 space-y-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {pipelineProgress && (<p className="text-[11px] text-muted-foreground mt-1 truncate" title={pipelineProgress}>
                  {pipelineProgress}
                </p>)}
            </div>
          </div>
        ...
        {!isContabilidade && (<div className="text-center">
            <button type="button" onClick={() => setShowDetails(v => !v)} className="text-xs text-[hsl(258,90%,66%)] hover:underline inline-flex items-center gap-1">
              {showDetails ? "Ocultar etapas detalhadas" : "Ver etapas detalhadas"}
              <ChevronDown className={`w-3 h-3 transition-transform ${showDetails ? "rotate-180" : ""}`}/>
            </button>
          </div>)}

        {showDetails && (<div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
            {processingSteps.map((step, i) => (<div key={i} className={`flex items-center gap-3 p-2.5 rounded-lg transition-all ${i < currentStep ? "bg-emerald-500/5" :
                    i === currentStep ? "bg-[hsl(258,90%,66%)]/5 border border-[hsl(258,90%,66%)]/20" :
                        "opacity-40"}`}>
                {i < currentStep ? (<CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0"/>) : i === currentStep ? (<Loader2 className="w-4 h-4 text-[hsl(258,90%,66%)] animate-spin shrink-0"/>) : (<div className="w-4 h-4 rounded-full border border-border shrink-0"/>)}
...
const FormulaInfo = ({ title, formula, accounts }) => (<Popover>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {(d.pontosChave || []).map((p) => (<div key={p.item} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/30">
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${p.status === "positivo" ? "bg-emerald-500" :
 ...
 */
const computeIndicatorsFromParsed = (parsed) => {
    if (!parsed)
        return {};
...
const computeIndicatorsFromBSRows = (rows) => {
...
const TabIndicadores = ({ parsedData, aiAnalysis, bsRows }) => {
    const { state } = useAudit();
    const computedInd = useMemo(() => (bsRows && bsRows.length > 0 ? computeIndicatorsFromBSRows(bsRows) : computeIndicatorsFromParsed(parsedData || null)), [bsRows, parsedData]);
    ...
    const hasAiInd = aiInd && Object.values(aiInd).some((v) => v !== 0);
    let ind;
    let years;
    if (hasComputed) {
        ind = computedInd;
    ...
    const sections = [
        {
            ...
            items: [
                { label: "Liquidez Corrente", key: "liquidezCorrente", fmt: (n) => (n ?? 0).toFixed(2), formula: "AC / PC", benchmark: "> 1,5", accounts: ["Ativo Circulante (Grupo 1.1)", "Passivo Circulante (Grupo 2.1)"] },
                { label: "Liquidez Seca", key: "liquidezSeca", fmt: (n) => (n ?? 0).toFixed(2), formula: "(AC - EST) / PC", benchmark: "> 1,0", accounts: ["Ativo Circulante (Grupo 1.1)", "Estoques (Ref 1: D)", "Passivo Circulante (Grupo 2.1)"] },
                { label: "Liquidez Imediata", key: "liquidezImediata", fmt: (n) => (n ?? 0).toFixed(2), formula: "Caixa / PC", benchmark: "> 0,3", accounts: ["Disponibilidades (Refs 1: A, B)", "Passivo Circulante (Grupo 2.1)"] },
                { label: "Liquidez Geral", key: "liquidezGeral", fmt: (n) => (n ?? 0).toFixed(2), formula: "(AC + RLP) / (PC + PNC)", benchmark: "> 0,1", accounts: ["Ativo Circulante (Grupo 1.1)", "Realizável a Longo Prazo (Grupo 1.2.1)", "Passivo Circulante (Grupo 2.1)", "Passivo Não Circulante (Grupo 2.2)"] },
            ]
        },
        {
            ...
            items: [
                ...
                { label: "Cobertura de Juros", key: "coberturaJuros", fmt: (n) => `${(n ?? 0).toFixed(1)}x`, formula: "LAJIR / Juros", benchmark: "> 3,0x", accounts: ["Resultado Operacional", "Despesas Financeiras (Grupo 7)"] },
            ]
        },
        {
    ...
    return (<div className="space-y-4">
        ...
        {sections.map(sec => (<Card key={sec.title}>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {sec.items.map(item => (<TableRow key={item.key}>
                      <TableCell className="text-xs font-medium">
                        <div className="flex items-center gap-1.5">
                          {item.label}
                          <FormulaInfo title={item.label} formula={item.formula} accounts={item.accounts}/>
                        </div>
                      </TableCell>
                      <TableCell className="text-[10px] text-muted-foreground font-mono">{item.formula}</TableCell>
                      {years.map(y => (<TableCell key={y} className="text-right text-xs font-mono">
                          {ind[y] ? item.fmt(ind[y][item.key]) : "—"}
                        </TableCell>))}
      ...
      {hasComputed && years.length > 0 && (<Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <Calculator className="w-4 h-4 text-accent"/> 
            EBITDA Estimado
            <FormulaInfo title="EBITDA Estimado" formula="LAJIR + Depreciação + Amortização" accounts={["Resultado Operacional (LAJIR)", "Despesas Financeiras (Grupo 7)", "Depreciação/Amortização (quando disponível)"]}/>
          </CardTitle>
        </CardHeader>
          <CardContent>
            <div className={`grid grid-cols-${Math.min(years.length, 4)} gap-4`}>
              {years.map(y => {
                ...
                return (<div key={y} className="p-4 rounded-lg bg-muted/30 text-center">
...
const TabEndividamento = ({ aiAnalysis, parsedData, bsRows }) => {
    const computedInd = useMemo(() => (bsRows && bsRows.length > 0 ? computeIndicatorsFromBSRows(bsRows) : computeIndicatorsFromParsed(parsedData || null)), [bsRows, parsedData]);
    ...
    return (<div className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        ...
        ].map(item => (<div key={item.label} className={`flex justify-between p-3 rounded-lg ${item.highlight ? "bg-accent/5 border border-accent/20" : "bg-muted/30"}`}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="border-t border-border/50 pt-3 space-y-2">
              ...
              {riscos.map((r) => (<div key={r.tipo} className="flex items-start gap-2 p-2 rounded bg-muted/20">
...
const TabPatrimonial = ({ aiAnalysis, parsedData, bsRows }) => {
    const { state } = useAudit();
    ...
    return (<div className="space-y-4">
            ...
            return (<Table>
                ...
                <TableBody>
                  {groupedRows.map((row, idx) => {
                    ...
                    return (<TableRow key={`${row.conta}-${idx}`} className={`${bg} ${row.hasRisk ? "bg-orange-500/5" : ""}`}>
                        <TableCell className={`text-xs ${indent} ${weight}`}>{row.descricao}</TableCell>
                        {years.map(y => (<TableCell key={y} className={`text-right text-xs font-mono ${isGroup ? "font-bold" : isSubgroup ? "font-semibold" : ""}`}>{fmt(row.values[y] || 0)}</TableCell>))}
                        {prevYear && (<TableCell className={`text-right text-xs font-mono ${isAlert ? "text-orange-500 font-bold" : ""}`}>
                            {ah > 0 ? "+" : ""}{fmtPct(ah)}
                          </TableCell>)}
                      </TableRow>);
                })}
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {alertas.map((a, i) => (<div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/20 border border-border/30">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              {(activeScore.componentes || []).map((c) => (<div key={c.nome} className="space-y-1">
        ...
        ].map(item => (<div key={item.range} className={`flex items-center justify-between p-3 rounded-lg bg-muted/20 ${item.active ? "ring-2 ring-accent" : ""}`}>
                  <div className="flex items-center gap-3">
          ...
          <Card className="bg-muted/30">
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {["Lei 11.101/2005", "CPC 26", "NBC TA 570", "Princípio da Continuidade"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
              </div>
            </CardContent>
          </Card>
        </div>
...
const TabAnaliseTecnica = ({ pendenciasData, parsedData, isHistoricalView = false, company }) => {
    const activePendencias = pendenciasData || pendencias;
    const [selectedId, setSelectedId] = useState(activePendencias[0]?.id || "");
    const selected = activePendencias.find((p) => p.id === selectedId);
          ...
          <CardContent>
            <ScrollArea className="h-[360px]">
              <div className="space-y-2 pr-2">
                {activePendencias.map((p) => (<button key={p.id} onClick={() => setSelectedId(p.id)} className={`w-full text-left p-3 rounded-lg border transition-all ${selectedId === p.id ? "border-[hsl(258,90%,66%)] bg-[hsl(258,90%,66%)]/5" : "border-border/50 hover:bg-muted/50"}`}>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            {selected ? (<div className="space-y-4">
              ...
              </div>) : (<p className="text-sm text-muted-foreground">Selecione uma pendência para ver o parecer do Técnico IA.</p>)}
          </CardContent>
        </Card>
      </div>

      {/* Chat IA integrado — disponível apenas no dia da auditoria (oculto em relatórios históricos) */}
      {isHistoricalView ? (<Card className="border-dashed">
        ...
        </Card>) : (<Card className="border-[hsl(258,90%,66%)]/30 shadow-lg">
          <CardHeader className="pb-2 bg-[hsl(258,90%,66%)]/5 border-b border-[hsl(258,90%,66%)]/20">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {selected && (<Badge variant="secondary" className="text-[10px]">Conta {selected.conta}</Badge>)}
              </div>
            </div>
          ...
          <CardContent className="pt-4">
            <div ref={chatScrollRef} className="mb-3 overflow-y-auto rounded-lg border border-border/40 bg-background/60 p-3" style={{ height: 340 }}>
              <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-1.5">
                {["Por que classificou como crítico?", "Qual o impacto no balancete?", "Qual ajuste contábil sugere?", "Gera ressalva no parecer?"].map(q => (<button key={q} onClick={() => setChatInput(q)} className="text-[10px] px-2.5 py-1 rounded-full bg-muted/60 border border-border/60 text-foreground hover:bg-[hsl(258,90%,66%)]/10 hover:border-[hsl(258,90%,66%)]/40 transition-colors">
                    {q}
                  </button>))}
              </div>
...
const ReportPage = ({ children, className = "" }) => (<div className={`report-a4-page ${className}`} style={{ "--report-watermark": `url(${folhaRostoBg})` }}>
...
const TabRelatorioPreview = ({ onGerarBex, onGerarKanitz, selectedDepth = "tecnico" }) => {
        ...
        <CardContent className="space-y-4">
          <p className="text-xs text-muted-foreground leading-relaxed">
           ...
           <div className="flex justify-center gap-2">
             ...
             {bexAvailable && (<Badge className="bg-emerald-500/15 text-emerald-700 border-emerald-500/30 text-xs font-semibold px-3 py-1">Selecionado</Badge>)}
           </div>
          <div className="space-y-2">
            {reportTopicsBex.map(t => {
            const Icon = t.icon;
            return (<div key={t.num} className="flex items-start gap-3 p-2.5 rounded-lg bg-muted/20">
          ...
          <div className="flex flex-wrap gap-1.5">
            {["NBC TA 700", "CPC 26", "IFRS 15", "Lei 11.101/2005"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
          </div>
          <Button onClick={onGerarBex} className="w-full bg-[hsl(258,90%,66%)] hover:bg-[hsl(258,90%,56%)] text-white gap-2 h-11 text-sm font-semibold rounded-xl shadow-lg shadow-[hsl(258,90%,66%)]/20">
        ...
        <CardContent className="space-y-4">
          <p className="text-xs text-muted-foreground leading-relaxed">
           ...
           <div className="flex justify-center gap-2">
             ...
             {kanitzAvailable && (<Badge className="bg-emerald-500/15 text-emerald-700 border-emerald-500/30 text-xs font-semibold px-3 py-1">Selecionado</Badge>)}
           </div>
          <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {["Kanitz (1978)", "NBC TA 570", "Lei 11.101/2005", "CPC 26"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
          </div>
          <Button onClick={onGerarKanitz} className="w-full bg-amber-500 hover:bg-amber-600 text-white gap-2 h-11 text-sm font-semibold rounded-xl shadow-lg shadow-amber-500/20">
...
/* ══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
export const TabRelatorioFinal = ({ onBack, aiAnalysis, parsedData, onSwitchToKanitz, variant = "resumido", uploadedFiles, sourceDocs, company, balanceteEntries, processingRunId }) => {
    const { state } = useAudit();
    ...
    const computeIndicatorsFromParsed = useCallback((parsed) => {
        if (!parsed)
            return {};
    ...
    const snapshot = useMemo(() => buildCertifiedFinancialSnapshot(parsedData, balanceteEntries || [], {
    ...
    const activePend = recomputePendencyPercentages(filterStalePendencias(aiAnalysis?.pendencias || pendencias, snapshot?.facts), snapshot?.facts?.ativo_total);
    ...
    const canonicalKeyPoints = latestInd ? [
    ...
    const kanitzFindValue = (keyword, year) => {
        if (!parsedData)
            return 0;
        const allRows = [...parsedData.balanco, ...parsedData.dre];
    ...
    const fmtKDec = (n) => (n ?? 0).toFixed(4);
    const SectionTitle = ({ num, title }) => (<div className="flex items-center gap-3 py-3 border-b-2 border-[hsl(258,90%,66%)]/30 mb-4">
    ...
    return (<div ref={reportContainerRef} style={{ "--report-watermark": `url(${folhaRostoBg})` }} id="report-bex-container">
      ...
      <div className="flex items-center justify-between gap-2 print:hidden mb-4 flex-wrap">
        <div className="flex items-center gap-1.5">
          {totalPages > 0 && (<span className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mr-3">
      ...
      {/* ── CAPA A4 ── */}
      <div className="report-a4-cover" style={{ "--report-watermark": `url(${folhaRostoBg})` }}>
        {/* Header with logo */}
        <div className="report-page-header">
          <img src={logoBrasilExpertFull} alt="Brasil Expert" className="h-14 object-contain"/>
        </div>
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {canonicalKeyPoints.map((p) => (<div key={p.item} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/30">
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${p.status === "positivo" ? "bg-emerald-500" :
                p.status === "atencao" ? "bg-yellow-500" : "bg-red-500"}`}/>
          ...
          <div>
            ...
            <div className="p-4 rounded-lg bg-muted/30 border border-border/50">
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                {["CPC 26", "CPC 47", "IFRS 15", "NBC TA 570", "Lei 11.101/2005"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
              </div>
            </div>
          </div>
        </div>
      ...
      <ReportPage>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {solvencyIndicators.map(si => (<TableRow key={si.name}>
                      ...
                      <TableCell>
                        <Badge className={`text-[10px] ${si.classification === "Adequada" || si.classification === "Positivo" || si.classification === "Solvente"
                ? "bg-emerald-500/15 text-emerald-600"
                : si.classification === "Atenção"
                    ? "bg-yellow-500/15 text-yellow-600"
                    : "bg-red-500/15 text-red-600"}`}>{si.classification}</Badge>
                      </TableCell>
            ...
            {solvencyIndicators.length > 0 && (<div className="mt-4">
                ...
                value: parseFloat(si.result.replace("%", "").replace(",", ".")) || 0,
                      ...
                      <Tooltip formatter={(v) => [`${(v ?? 0).toFixed(1)}%`, "Resultado"]}/>
                      <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                        {solvencyIndicators.filter(si => !si.name.includes("Capital") && !si.name.includes("Cobertura")).map((_, i) => (<Cell key={i} fill={["hsl(258,90%,66%)", "hsl(258,70%,60%)", "hsl(258,50%,55%)", "hsl(258,90%,50%)"][i % 4]}/>))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
        ...
        ].map(item => (<div key={item.title} className="p-3 rounded-lg bg-muted/20 border border-border/30 report-card-keep-together mb-2 break-inside-avoid" style={{ pageBreakInside: 'avoid', breakInside: 'avoid-page' }}>
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          <div>
            ...
            const factAvailable = (v) => typeof v === "number" && Number.isFinite(v) && v !== 0;
            const isInternalError = txt.includes("fatores determinísticos") ||
          ...
          <div>
            ...
            <div className="space-y-3 report-keep-together break-inside-avoid">
              {activePend.map((p, i) => (<div key={p.id} className="p-4 rounded-lg border border-border/50 space-y-2 report-card-keep-together break-inside-avoid" style={{ pageBreakInside: 'avoid', breakInside: 'avoid-page' }}>
                  <div className="flex items-center gap-2">
                    <Badge className={`${severityColors[p.gravidade]?.bg} text-[10px]`}>{severityColors[p.gravidade]?.label}</Badge>
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          <div>
        ...
        ].map(item => (<TableRow key={item.name}>
            ...
            {years.length > 0 && (<div className="mt-4">
            ...
            {years.length > 0 && (<div className="mt-4">
                ...
                return {
                    ...
                    "TOTAL": total / 1000,
                      ...
                      <Tooltip formatter={(v) => `R$ ${(v * 1000).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}`}/>
                      <Legend wrapperStyle={{ fontSize: 9 }}/>
                      ...
                      <Line yAxisId="right" type="linear" dataKey="TOTAL" stroke="#c00000" strokeWidth={2.5} dot={{ r: 4, fill: "#c00000" }}>
                        <LabelList dataKey="TOTAL" position="top" fontSize={9} fill="#c00000" formatter={(v) => (v * 1000).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}/>
                      </Line>
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
        ...
        ].map(item => (<div key={item.label} className="p-3 rounded-lg bg-muted/30 border border-border/30 break-inside-avoid">
            ...
            {years.length > 0 && (<div className="mt-4">
                ...
                return {
                    ...
                    pct: parseFloat(((pct) ?? 0).toFixed(2)),
                      ...
                      <XAxis dataKey="name" tick={{ fontSize: 9 }}/>
                      <YAxis tick={{ fontSize: 9 }} tickFormatter={(v) => v.toLocaleString('pt-BR')}/>
                      <Tooltip formatter={(v, n) => n === "pct" ? `${(v ?? 0).toFixed(2)}%` : `R$ ${(v * 1000).toLocaleString('pt-BR')}`}/>
                      <Legend wrapperStyle={{ fontSize: 9 }}/>
                      <Bar dataKey="Receita Líquida" fill="#5b9bd5">
                        <LabelList dataKey="Receita Líquida" position="top" fontSize={9} formatter={(v) => v.toLocaleString('pt-BR')}/>
                      </Bar>
                      <Bar dataKey="CMV + DESPESA / RECEITA LÍQUIDA" fill="#c00000">
                        <LabelList dataKey="CMV + DESPESA / RECEITA LÍQUIDA" position="bottom" fontSize={9} formatter={(v) => `(${Math.abs(v).toLocaleString('pt-BR')})`}/>
                      </Bar>
                      <Line type="linear" dataKey="pct" name="CMV + DESPESA / RECEITA LÍQUIDA (%)" stroke="#c00000" strokeWidth={0} dot={false}>
                        <LabelList dataKey="pct" position="top" fontSize={9} fill="#c00000" formatter={(v) => `${(v ?? 0).toFixed(2)}%`}/>
                      </Line>
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
      ...
      <ReportPage>
        ...
        ].map(item => (<div key={item.label} className={`flex justify-between p-3 rounded-lg ${item.risk ? "bg-orange-500/5 border border-orange-500/20" : "bg-muted/20"}`}>
          ...
          <div>
            ...
            <div className="p-4 rounded-lg bg-muted/30 border border-border/50 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5 pt-2">
                {["Risco de vencimento concentrado", "Capacidade de renegociação limitada", "Monitorar covenants", "Avaliar necessidade de RJ"].map(t => (<Badge key={t} variant="outline" className="text-[10px]">{t}</Badge>))}
              </div>
            </div>
          </div>
        </div>
      ...
      <ReportPage className={years.length > 6 ? "landscape" : ""}>
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <Table style={{ tableLayout: 'fixed', width: '100%' }}>
              <TableHeader>
                <TableRow>
                  ...
                  {(snapshot?.competencies || []).map(y => (<TableHead key={y} className="text-right text-[10px] px-1">{fmtMonthCompact(y)}</TableHead>))}
                </TableRow>
              </TableHeader>
              <TableBody>
                {[
            ...
            { label: "Resultado da Competência", key: "resultado_competencia" },
            { label: "Resultado Acumulado", key: "resultado_acumulado" },
            { label: "Trabalhista (CP)", key: "divida_trabalhista" },
            { label: "Fornecedores (CP)", key: "fornecedores" },
            { label: "Fornecedores (LP)", key: "fornecedores_lp" },
        ].map((row, idx) => (<TableRow key={row.label} className={idx % 2 === 0 ? "bg-muted/10" : ""}>
                ...
                const factVal = comp?.facts ? comp.facts[row.key] : undefined;
                const statusOk = comp?.facts_status
                    ? comp.facts_status[row.key] !== "NOT_AVAILABLE"
                    : true;
                const residualVal = comp?.residual?.[row.key];
        ...
        ].map(v => (<div key={v.check} className={`flex items-center justify-between p-3 rounded-lg border ${v.status ? "bg-emerald-500/5 border-emerald-500/20" : "bg-red-500/5 border-red-500/20"}`}>
                  <div className="flex items-center gap-2">
      ...
      {BEX_INCLUDE_KANITZ && latestKanitz && (<div className="report-a4-cover" style={{ "--report-watermark": `url(${folhaRostoBg})` }}>
          <div className="report-page-header">
            <img src={logoBrasilExpertFull} alt="Brasil Expert" className="h-14 object-contain"/>
          </div>
          <div className="px-12 text-center mt-8">
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-12 text-center">
            ...
            <div className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full border ${!latestKanitz.kanitzAplicavel ? "border-slate-500/30 bg-slate-500/5" : "border-amber-500/30 bg-amber-500/5"} mt-8`}>
              ...
              <span className="text-sm font-semibold text-foreground">
                {!latestKanitz.kanitzAplicavel ? `NÃO APLICÁVEL | PL: R$ ${fmt(latestKanitz.pl)}` : `${kanitzClassColors[latestKanitz.classificacao]?.label} — FI: ${fiFmt(latestKanitz.fi, latestKanitz.kanitzAplicavel)}`}
              </span>
            </div>
      ...
      {BEX_INCLUDE_KANITZ && latestKanitz && (<ReportPage>
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-lg bg-muted/30 border border-border/50">
              <p className="text-sm text-foreground leading-relaxed">
                {!latestKanitz.kanitzAplicavel ? (`O modelo Kanitz não é aplicável para o período selecionado, pois o Patrimônio Líquido (R$ ${fmt(latestKanitz.pl)}) é nulo ou negativo. Nestes casos, o indicador FI perde a validade estatística. Recomenda-se a análise via Solvência Total (ISG: ${latestKanitz.isg.toFixed(2)}).`) : (`A empresa apresenta Fator de Insolvência de ${fiFmt(latestKanitz.fi, latestKanitz.kanitzAplicavel)}, classificando-se como ${kanitzClassColors[latestKanitz.classificacao]?.label?.toUpperCase()} segundo o modelo Kanitz. ${(latestKanitz.fi ?? 0) > 0 ? "Os indicadores de liquidez e rentabilidade demonstram solidez financeira e capacidade plena de honrar obrigações." : (latestKanitz.fi ?? 0) > -3 ? "Os indicadores financeiros demonstram fragilidades que requerem monitoramento contínuo e medidas preventivas." : "A deterioração severa dos indicadores financeiros indica incapacidade de pagamento. Recomenda-se análise de viabilidade conforme Lei 11.101/2005."}`)}
              </p>
            </div>
            <div className="grid sm:grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-muted/20 text-center">
                <p className="text-[10px] text-muted-foreground">Pontuação Kanitz</p>
                <p className={`text-2xl font-bold font-mono ${kanitzClassColors[latestKanitz.classificacao]?.color}`}>{fiFmt(latestKanitz.fi, latestKanitz.kanitzAplicavel)}</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/20 text-center">
                ...
                <div className="h-[220px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={kanitzResults.map(r => ({ name: r.year, FI: parseFloat(((r.fi) ?? 0).toFixed(2)) }))} margin={{ top: 25, right: 20, left: 0, bottom: 5 }}>
                      ...
                      <Tooltip formatter={(v) => [((v) ?? 0).toFixed(2), "Fator de Insolvência"]}/>
                      <Line type="linear" dataKey="FI" stroke="#ed7d31" strokeWidth={2.5} dot={{ r: 4, fill: "#ed7d31" }}>
                        <LabelList dataKey="FI" position="top" fontSize={10} fill="#ed7d31" formatter={(v) => ((v) ?? 0).toFixed(2)}/>
                      </Line>
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              ...
              <div>
                ...
                <div className="overflow-x-auto">
                  <Table style={{ tableLayout: 'fixed', width: '100%' }}>
                    ...
                    <TableBody>
                      {kanitzResults.map((r) => (<TableRow key={r.year}>
      ...
      <ReportPage>
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 pt-2">
            {["NBC TA 700", "NBC TA 705", "CPC 26", "Lei 11.101/2005", "IFRS", "Kanitz (1978)"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
          </div>
        </div>
      </ReportPage>
      ...
      <div className="flex items-center justify-center gap-3 pt-4 print:hidden flex-wrap">
        {totalPages > 0 && (<span className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
            <FileText className="w-3.5 h-3.5"/> Total de páginas: {totalPages}
          </span>)}
        <div className="inline-flex items-center rounded-lg border border-border bg-muted/50 p-0.5">
          <div className="relative">
            ...
            const bexTab = tabList?.querySelector('[value="relatorio-final"]');
            bexTab?.click();
        }}>
              <BookOpen className="w-3.5 h-3.5"/> Relatório BEX
            </button>
...
/* ══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
const TabRelatorioKanitz = ({ onBack, parsedData, onSwitchToBex, aiAnalysis, uploadedFiles, sourceDocs, company, reportDataset }) => {
    const today = new Date().toLocaleDateString("pt-BR");
    const kanitzContainerRef = useRef(null);
    ...
    const SectionTitle = ({ num, title }) => (<div className="flex items-center gap-3 py-3 border-b-2 border-amber-500/30 mb-4">
      <div className="w-8 h-8 rounded-lg bg-amber-500 text-white flex items-center justify-center text-sm font-bold">{num}</div>
      <h2 className="text-lg font-bold text-foreground font-serif">{title}</h2>
    </div>);
    const fmtDec = (n) => (n ?? 0).toFixed(4);
    const fmtOrNA = (n, suffix = "", isApplicable = true) => !isApplicable ? "N/A" : (typeof n === "number" && isFinite(n) ? `${(n ?? 0).toFixed(2)}${suffix}` : "N/A");
    ...
    const simCompute = (pl2, pt2, ac2, pc2, est2, rlp2, ll2) => {
        const kAplic2 = pl2 > 0;
        if (!kAplic2)
            return 0;
    ...
    return (<div ref={kanitzContainerRef} className="space-y-0" style={{ "--report-watermark": `url(${folhaRostoBg})` }} id="report-kanitz-container">
      {/* Action buttons */}
      <div className="flex items-center justify-between gap-2 print:hidden mb-4 flex-wrap">
        <div className="flex items-center gap-1.5">
          {totalPagesKanitz > 0 && (<span className="text-xs font-medium text-muted-foreground flex items-center gap-1.5 mr-3">
      ...
      <div className="report-a4-cover" style={{ "--report-watermark": `url(${folhaRostoBg})` }}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-12 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-amber-500/30 bg-amber-500/5 mt-8">
            ...
            <span className="text-sm font-semibold text-foreground">
              {kAplic
            ? `${classColors[l.classificacao]?.label} — FI: ${fiFmt(l.fi, kAplic)}`
      ...
      <ReportPage>
        <div className="space-y-4 break-inside-avoid">
          ...
          <div className="p-4 rounded-lg bg-muted/30 border border-border/50">
            ...
            ? `A empresa apresenta Fator de Insolvência de ${fiFmt(l.fi, kAplic)}, classificando-se como SAUDÁVEL segundo o modelo Kanitz. Os indicadores de liquidez e rentabilidade demonstram solidez financeira e capacidade plena de honrar obrigações.`
            : l.classificacao === "estavel"
                ? `A empresa apresenta Fator de Insolvência de ${fiFmt(l.fi, kAplic)}, classificando-se como ESTÁVEL. A estrutura financeira é adequada, com indicadores dentro de parâmetros aceitáveis. Recomenda-se manutenção das políticas financeiras atuais.`
                : l.classificacao === "atencao"
                    ? `A empresa encontra-se em ZONA DE ATENÇÃO com FI de ${fiFmt(l.fi, kAplic)}. Indicadores de liquidez e endividamento apresentam fragilidades que requerem monitoramento contínuo.`
                    : l.classificacao === "risco"
                        ? `A empresa está em ZONA DE RISCO com FI de ${fiFmt(l.fi, kAplic)}. Os indicadores financeiros demonstram deterioração significativa. Liquidez Seca de ${fmtDec(reportDataset?.ratios.liquidezSeca || 0)} e Grau de Endividamento de ${fmtDec(reportDataset?.ratios.endividamentoTotal || 0)} indicam dificuldades financeiras. Recomenda-se reestruturação imediata.`
                        : `A empresa apresenta ALTA PROBABILIDADE DE INSOLVÊNCIA com FI de ${fiFmt(l.fi, kAplic)}. A deterioração severa dos indicadores financeiros indica incapacidade de pagamento. Recomenda-se análise de viabilidade conforme Lei 11.101/2005.`}
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          {!kAplic && (<div className="p-4 rounded-lg border border-red-500/40 bg-red-500/5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="text-center py-6 rounded-lg bg-muted/20">
              ...
              <p className={`text-5xl font-bold ${kAplic ? classColors[l.classificacao]?.color : "text-slate-400 line-through"}`}>{fiFmt(l.fi, kAplic)}</p>
          ...
          <div className="px-4">
            ...
            return (<div key={r.year} className="absolute top-0 bottom-0 w-1 bg-foreground rounded-full shadow-lg" style={{ left: `${pos}%`, transform: "translateX(-50%)" }} title={`${r.year}: FI = ${fiFmt(r.fi, r.kanitzAplicavel)}`}>
          ...
          <div className="grid sm:grid-cols-3 gap-3 mt-4">
            {kanitzResults.map(r => (<div key={r.year} className={`p-4 rounded-lg border text-center space-y-1 ${!r.kanitzAplicavel ? "bg-slate-500/10 border-slate-500/30" :
                ...
                {r.kanitzAplicavel ? (<>
                    <p className="text-2xl font-bold font-mono">{fiFmt(r.fi, r.kanitzAplicavel)}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <Table className="table-fixed w-full border-collapse">
              ...
              <TableBody>
                {[
            { name: "Rentabilidade do PL", sigla: "RPL", formula: "LL / PL", peso: 0.05, key: "rpl", naIfNegativePL: true },
            { name: "Liquidez Geral", sigla: "LG", formula: "(AC + RLP) / PT", peso: 1.65, key: "lg", naIfNegativePL: false },
            { name: "Liquidez Seca", sigla: "LS", formula: "(AC − EST) / PC", peso: 3.55, key: "ls", naIfNegativePL: false },
            { name: "Liquidez Corrente", sigla: "LC", formula: "AC / PC", peso: -1.06, key: "lc", naIfNegativePL: false },
            { name: "Grau de Endividamento", sigla: "GE", formula: "PT / PL", peso: -0.33, key: "ge", naIfNegativePL: true },
        ].map(ind => (<TableRow key={ind.sigla} className="h-7">
                    ...
                    {kanitzResults.map(r => (<TableCell key={r.year} className="text-right text-[10px] py-0.5 border border-border font-mono">
                ...
                <TableRow className="bg-amber-500/5 h-7">
                  ...
                  {kanitzResults.map(r => (<TableCell key={r.year} className="text-right text-[10px] py-0.5 border border-border font-mono font-bold">{(r.isg ?? 0).toFixed(2)}</TableCell>))}
                </TableRow>
                <TableRow className="border-t-2 border-foreground/20 h-8">
                  ...
                  {kanitzResults.map(r => (<TableCell key={r.year} className={`text-right text-[11px] py-1 border border-border font-bold font-mono ${classColors[r.classificacao]?.color}`}>
                      {r.kanitzAplicavel ? fiFmt(r.fi) : <span className="text-slate-500">N/A</span>}
                    </TableCell>))}
                </TableRow>
              </TableBody>
            </Table>
          </div>
          {!kAplic && (<p className="text-[10px] text-muted-foreground italic">
      ...
      <ReportPage>
        ...
        ].map(item => (<div key={item.label} className={`p-3 rounded-lg border text-center space-y-1 ${item.status === "positivo" ? "bg-emerald-500/5 border-emerald-500/20" :
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          {!kAplic && (<p className="text-[10px] text-red-600 italic">
        ...
        ].map(item => (<div key={item.range} className={`flex items-center justify-between p-2.5 rounded-lg bg-muted/20 ${item.active ? "ring-2 ring-amber-500" : ""}`}>
                <div className="flex items-center gap-3">
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          {l.pnc === 0 && (<div className="p-2 rounded bg-yellow-500/5 border border-yellow-500/20">
        ...
        ].map(item => (<div key={item.label} className={`p-2 rounded-lg ${item.alert ? "bg-red-500/5 border border-red-500/20" : "bg-background"} break-inside-avoid`}>
                  <div className="flex justify-between text-[10px]">
                    <span className="font-medium text-foreground">{item.label}</span>
                    <span className="font-mono font-bold">{!Number.isFinite(item.value) ? "N/A" : `${item.value.toFixed(item.suffix === "x" ? 2 : 1)}${item.suffix}`}</span>
                  </div>
      ...
      <ReportPage>
        ...
        ].map(item => (<div key={item.label} className={`p-3 rounded-lg border text-center space-y-1 ${item.alert ? "bg-red-500/5 border-red-500/20" : "bg-muted/20 border-border/30"}`}>
                <p className="text-[10px] text-muted-foreground">{item.label}</p>
                <p className="text-lg font-bold font-mono text-foreground">
      ...
      <ReportPage>
        <div className="space-y-4">
          ...
          {!kAplic ? (<div className="p-4 rounded-lg border border-slate-500/30 bg-slate-500/5 text-xs text-muted-foreground leading-relaxed">
              ...
              <TableBody>
                ...
                return (<TableRow key={sim.cenario}>
                      ...
                      <TableCell className="text-xs">
                        <Badge className={`text-[9px] ${newClass === "saudavel" ? "bg-emerald-500/15 text-emerald-600" :
                        newClass === "estavel" ? "bg-blue-500/15 text-blue-600" :
                            newClass === "atencao" ? "bg-yellow-500/15 text-yellow-600" :
                                newClass === "risco" ? "bg-orange-500/15 text-orange-600" :
                                    newClass === "na" ? "bg-slate-500/15 text-slate-600" : "bg-red-500/15 text-red-600"}`}>{classColors[newClass]?.icon} {classColors[newClass]?.label}</Badge>
                      </TableCell>
      ...
      <ReportPage>
        ...
        ].map(item => (<div key={item.title} className="p-4 rounded-lg bg-muted/20 border border-border/30">
      ...
      <ReportPage>
        ...
        ].map(tbl => (<div key={tbl.title} className="mb-4 break-inside-avoid" style={{ breakInside: "avoid", pageBreakInside: "avoid" }}>
              ...
              <Table style={{ tableLayout: "fixed", width: "100%", wordBreak: "normal" }}>
                <TableHeader>
                  <TableRow className="bg-muted/30">
                    ...
                    {kanitzResults.map(r => (<TableHead key={r.year} className="text-right text-[10px]" style={{ width: `${66 / Math.max(kanitzResults.length, 1)}%` }}>
                ...
                <TableBody>
                  {[
                { name: "RPL (X1)", peso: 0.05, key: "rpl", dependsOnPL: true },
                { name: "LG (X2)", peso: 1.65, key: "lg", dependsOnPL: false },
                { name: "LS (X3)", peso: 3.55, key: "ls", dependsOnPL: false },
                { name: "LC (X4)", peso: -1.06, key: "lc", dependsOnPL: false },
                { name: "GE (X5)", peso: -0.33, key: "ge", dependsOnPL: true },
            ].map(c => (<TableRow key={c.name}>
                      ...
                      {kanitzResults.map(r => (<TableCell key={r.year} className="text-right text-[10px] font-mono">
                          {/* §50/§61 — apenas componentes dependentes do PL viram N/A; liquidez permanece publicada. */}
                          {(!r.kanitzAplicavel && c.dependsOnPL) || !Number.isFinite(r[c.key])
                        ? "N/A"
                        : tbl.weighted
                            ? (!r.kanitzAplicavel ? "N/A" : (c.peso * r[c.key]).toFixed(4))
                            : fmtDec(r[c.key])}
                        </TableCell>))}
                    </TableRow>))}
                  {tbl.weighted && (<TableRow className="border-t-2 border-foreground/20">
                      <TableCell className="text-[10px] font-bold" colSpan={2}>FATOR DE INSOLVÊNCIA (FI)</TableCell>
                      {kanitzResults.map(r => (<TableCell key={`fi-${r.year}`} className={`text-right text-[11px] font-bold font-mono ${classColors[r.classificacao]?.color || ""}`}>
                          {fiFmt(r.fi, r.kanitzAplicavel)}
                        </TableCell>))}
                    </TableRow>)}
                  {tbl.weighted && (<TableRow className="bg-amber-500/5">
                      <TableCell className="text-[10px] font-bold" colSpan={2}>ISG (AT / PT)</TableCell>
                      {kanitzResults.map(r => (<TableCell key={`isg-${r.year}`} className="text-right text-[10px] font-mono">{(r.isg ?? 0).toFixed(2)}</TableCell>))}
                    </TableRow>)}
                </TableBody>
              </Table>
            </div>))}
        ...
        ].map(v => (<TableRow key={v.label}>
                      <TableCell className="text-xs font-medium">{v.label}</TableCell>
                      {kanitzResults.map(r => (<TableCell key={r.year} className="text-right text-xs font-mono">R$ {fmt(r[v.key])}</TableCell>))}
                    </TableRow>))}
                </TableBody>
              </Table>
            </div>
      ...
      <ReportPage>
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 pt-2">
            {["Kanitz (1978)", "NBC TA 570", "CPC 26", "Lei 11.101/2005", "IFRS", "NBC TA 315"].map(n => (<Badge key={n} variant="secondary" className="text-[10px]">{n}</Badge>))}
          </div>
        </div>
      </ReportPage>
      ...
      {/* Action buttons bottom */}
      <div className="flex items-center justify-center gap-3 pt-4 print:hidden flex-wrap">
        {totalPagesKanitz > 0 && (<span className="text-xs font-medium text-muted-foreground flex items-center gap-1.5">
            <FileText className="w-3.5 h-3.5"/> Total de páginas: {totalPagesKanitz}
          </span>)}
        <div className="inline-flex items-center rounded-lg border border-border bg-muted/50 p-0.5">
          <div className="relative">
...
export const ResultsPhase = ({ onBack, aiAnalysis, parsedData, batchId, sourceDocs, company, source, uploadedFiles, selectedDepth = "tecnico", balanceteEntries = [], skipPersist = false, initialReportType, availableReports, processingRunId }) => {
    ...
    const [reportType, setReportType] = useState(initialReportType ?? (isResumido ? "bex" : "none"));
    // No fluxo técnico (Kanitz), ambos relatórios ficam disponíveis para seleção.
    // No fluxo executivo (BEX gratuito), apenas BEX.
    const effectiveAvailableReports = availableReports && availableReports.length > 0
        ? availableReports
    ...
    const [maxUnlocked, setMaxUnlocked] = useState(initialReportType ? tabOrder.length - 1 : currentIdx);
    useEffect(() => {
        if (currentIdx > maxUnlocked)
            setMaxUnlocked(currentIdx);
    ...
    const activePendencias = useMemo(() => recomputePendencyPercentages(filterStalePendencias(rawPendencias, reportDataset?.facts), reportDataset?.facts?.ativo_total), [rawPendencias, reportDataset]);
    const persistReport = (variant) => {
        ...
        const detRiskLevel = det?.risk_level;
        ...
        const conformidade = detConformidade ?? (riskLevel === "baixo" ? 95 : riskLevel === "moderado" ? 78 : riskLevel === "elevado" ? 55 : 35);
        const baseName = parsedData?.fileName || aiAnalysis?.fileName || "Auditoria";
        ...
        const entry = {
            ...
            balanceteEntries: (balanceteEntries || []).map(e => {
                if (e.mesReferencia && e.mesReferencia !== "auto")
                    return e;
            ...
            periodos: (() => {
                const fromUser = (balanceteEntries || [])
                    .map(e => e.mesReferencia)
                    .filter((m) => !!m && m !== "auto" && /^\d{4}-(0[1-9]|1[0-2])$/.test(m))
                    .map(m => m.slice(0, 7));
    ...
    if (isResumido) {
        return (<div className="space-y-6 bex-results-wrapper">
    ...
    return (<div className="space-y-6 bex-results-wrapper">
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-56 p-2">
              <div className="space-y-1">
                ...
                <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-xs h-9" onClick={() => exportPdf('report-bex-container', 'Relatório BEx')}>
                  <FileText className="w-3.5 h-3.5 text-blue-500"/> 
                  Relatório BEx (PDF)
                </Button>

                {!isResumido && (<Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-xs h-9" onClick={() => exportPdf('report-kanitz-container', 'Relatório Kanitz')}>
                    <FileText className="w-3.5 h-3.5 text-amber-500"/> 
                    Relatório Kanitz (PDF)
                  </Button>)}
                
                <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-xs h-9" onClick={() => exportPdf('tab-graficos-container', `BEx_Graficos_Auditoria_e_Parecer_Contabil_${new Date().toISOString().split('T')[0]}`)}>
                ...
                <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-xs h-9" onClick={() => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        ].map(({ value, icon: Icon, label }, idx) => (<TabsTrigger key={value} value={value} className={`text-xs gap-1.5 relative rounded-t-md border-t-2 border-x-2 border-transparent transition-colors data-[state=active]:bg-[hsl(258,90%,66%)] data-[state=active]:text-white data-[state=active]:border-[hsl(258,90%,66%)] ${idx < currentIdx ? "border-t-[hsl(258,90%,66%)]" : ""} ${idx > effectiveMax ? "hidden" : ""}`}>
        ...
        <TabsContent value="relatorio-final">
          {effectiveAvailableReports.length > 0 && (<div className="flex flex-wrap items-center gap-2 mb-4 p-3 rounded-lg border bg-muted/30">
              <span className="text-xs font-semibold text-muted-foreground mr-1">Relatórios disponíveis:</span>
              {effectiveAvailableReports.includes("bex") && (<Button size="sm" variant={reportType === "bex" ? "default" : "outline"} className={`gap-1.5 h-8 ${reportType === "bex" ? "bg-[hsl(258,90%,66%)] hover:bg-[hsl(258,90%,56%)] text-white" : ""}`} onClick={() => setReportType("bex")}>
              ...
              {effectiveAvailableReports.includes("kanitz") && (<Button size="sm" variant={reportType === "kanitz" ? "default" : "outline"} className={`gap-1.5 h-8 ${reportType === "kanitz" ? "bg-amber-500 hover:bg-amber-600 text-white" : ""}`} onClick={() => setReportType("kanitz")}>
                  <Scale className="w-3.5 h-3.5"/>
                  Relatório Kanitz
                  <Badge className="ml-1 bg-emerald-500/20 text-emerald-700 border-emerald-500/30 text-[9px] px-1.5 py-0">Disponível</Badge>
                </Button>)}
            </div>)}
          {reportType === "bex" ? (<TabRelatorioFinal onBack={onBack} aiAnalysis={aiAnalysis} parsedData={parsedData} variant="resumido" uploadedFiles={uploadedFiles} sourceDocs={sourceDocs} company={company} balanceteEntries={balanceteEntries} processingRunId={runId}/>) : reportType === "kanitz" ? (<TabRelatorioKanitz onBack={onBack} aiAnalysis={aiAnalysis} parsedData={parsedData} uploadedFiles={uploadedFiles} sourceDocs={sourceDocs} company={company} reportDataset={reportDataset}/>) : (<TabRelatorioPreview onGerarBex={handleGerarBex} onGerarKanitz={handleGerarKanitz} selectedDepth={selectedDepth}/>)}
        </TabsContent>
      </Tabs>

      {/* Botões laterais de navegação entre abas (wizard) — somem quando o relatório final é gerado */}
      {showWizardButtons && (<>
          {currentIdx > 0 && (<button type="button" onClick={goPrevTab} aria-label="Aba anterior" className="print:hidden fixed left-4 top-1/2 -translate-y-1/2 z-40 h-16 w-10 rounded-xl bg-[hsl(258,90%,66%)] text-white shadow-xl ring-2 ring-white/20 hover:bg-[hsl(258,90%,56%)] hover:scale-105 transition-all flex items-center justify-center">
              <ChevronLeft className="w-6 h-6"/>
            </button>)}
          <button type="button" onClick={goNextTab} disabled={currentIdx >= tabOrder.length - 1} aria-label="Próxima aba" className="print:hidden fixed right-4 top-1/2 -translate-y-1/2 z-40 h-16 w-10 rounded-xl bg-[hsl(12,90%,55%)] text-white shadow-xl ring-2 ring-white/20 hover:bg-[hsl(12,90%,45%)] hover:scale-105 transition-all flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100">
...
const AuditContent = () => {
    ...
    const [selectedDepth, setSelectedDepth] = useState("tecnico");
    const [multiMonth, setMultiMonth] = useState(null);
    const [filteredMonths, setFilteredMonths] = useState([]);
    const [monthsCap, setMonthsCap] = useState(3);
    const [monthsTier, setMonthsTier] = useState("gratuito");
    const [preParsing, setPreParsing] = useState(false);
    const [balanceteEntries, setBalanceteEntries] = useState([]);
    const [forceReprocess, setForceReprocess] = useState(false);
    const [processingRunId, setProcessingRunId] = useState(""); // MD-CUTOVER-001 §6
    const reportSource = role === "auditor_chefe" || role === "coordenadora" || role === "gestor_ia"
        ? "auditor_chefe"
    ...
    const handleAnalysisReady = useCallback((analysis, parsed) => {
    ...
    return (<PlatformLayout>
      <div className="max-w-[1400px] mx-auto p-4 md:p-6">
        {company && (<div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-[hsl(217,91%,50%)]/5 border border-[hsl(217,91%,50%)]/20">
        ...
        {phase === "upload" && (<UploadPhase onProcess={async () => {
                ...
                try {
                    ...
                    const userMonthByName = new Map(balanceteEntries.map(e => [e.fileName, e.mesReferencia ?? null]));
                    ...
                    if (merged.months.length > cap) {
                        toast({
                            ...
                            description: `Plano ${tier}. Detectamos ${merged.months.length} meses; apenas (${lastN.map(k => {
                                const m = merged.months.find(x => x.key === k);
                                return m?.label ?? k;
            ...
            }} onFilesReady={setUploadedFiles} onMesesReady={setBalanceteEntries} dedupConfig={dedupConfig} onDedupChange={setDedupConfig} onDepthChange={setSelectedDepth} onForceReprocess={setForceReprocess}/>)}
        <MonthsConfirmDialog open={phase === "confirm-months" && !!multiMonth} data={multiMonth} maxMonths={monthsCap} tier={monthsTier} onConfirm={(keys) => { setFilteredMonths(keys); setPhase("processing"); }} onCancel={() => { setMultiMonth(null); setPhase("upload"); }}/>

        {phase === "processing" && (<ProcessingPhase onComplete={() => setPhase("results")} files={uploadedFiles} preParsed={multiMonth ? pickMonths(multiMonth, filteredMonths) : null} onAnalysisReady={(analysis, parsed, runId) => {
                if (runId)
                    setProcessingRunId(runId);
                handleAnalysisReady(analysis, parsed);
            }} dedupConfig={dedupConfig} companyId={company?.id ?? null} balanceteEntries={balanceteEntries} forceReprocess={forceReprocess}/>)}
        {phase === "results" && (<ResultsPhase onBack={() => { setPhase("upload"); setAiAnalysis(null); setParsedData(null); setBatchId(""); setSourceDocs([]); setMultiMonth(null); }} aiAnalysis={aiAnalysis} parsedData={parsedData} batchId={batchId} sourceDocs={sourceDocs} company={company} source={reportSource} uploadedFiles={uploadedFiles} selectedDepth={selectedDepth} balanceteEntries={balanceteEntries} processingRunId={processingRunId}/>)}
      </div>
    </PlatformLayout>);
};
