...
const under = (n, prefix) => n.normalized_code === prefix || n.normalized_code.startsWith(prefix + ".");
const ref = (n) => ({
    code: n.account_code,
...
function pickNonOverlapping(nodes, match) {
    const matched = nodes.filter(match);
    return matched.filter(n => !matched.some(o => o !== n && n.normalized_code.startsWith(o.normalized_code + ".")));
}
/**
 ...
 */
function pickByTaxonomy(all, prefix, isTarget, isOther) {
    const inScope = all.filter(n => n.normalized_code.startsWith(prefix + "."));
    const childrenOf = (code) => inScope.filter(n => n.parent_code === code);
    const descendantsOf = (n) => inScope.filter(o => o.normalized_code.startsWith(n.normalized_code + "."));
    const out = [];
    const visit = (list) => {
        for (const n of list) {
            const desc = descendantsOf(n);
            const contaminated = desc.some(isOther) || (isOther(n) && !isTarget(n));
            if (isTarget(n) && !contaminated) {
                out.push(n);
                continue;
            }
            if (desc.length > 0) {
                visit(childrenOf(n.normalized_code));
                continue;
            }
            if (isTarget(n) && !isOther(n))
                out.push(n);
        }
    };
...
function compose(selected, scope, excluded = []) {
...
export function resolveResidualFacts(nodes, competency, ctx = {}) {
    ...
    /* ── Tributos (§33..§37) ────────────────────────────────── */
    const isTax = (n) => RX.tax.test(n.description) && !RX.labor.test(n.description);
    const isTaxInstallment = (n) => RX.installment.test(n.description) && isTax(n);
    const taxIn = (prefix) => pickNonOverlapping(liabilities.filter(n => under(n, prefix)), isTax);
    /** Parcelamentos tributários — descendentes dos grupos tributários já selecionados. */
    const instIn = (prefix, parents) => pickNonOverlapping(liabilities.filter(n => under(n, prefix) &&
        parents.some(p => n.normalized_code === p.normalized_code || n.normalized_code.startsWith(p.normalized_code + "."))), n => isTaxInstallment(n) && !parents.some(p => p.normalized_code === n.normalized_code));
    const isLaborNature = (n) => RX.labor.test(n.description) && !RX.tax.test(n.description);
    ...
    const tax = {
        ...
        total_exposure: taxCurrentNodes.length || taxNonCurrentNodes.length
            ? {
                ...
                calculation_scope: "Exposição tributária total = grupos tributários sintéticos CP + LP (parcelamentos contados uma única vez, como composição)",
            }
    ...
    /* ── Trabalhistas (§38..§40) ────────────────────────────── */
    const isLabor = (n) => RX.labor.test(n.description) &&
    ...
    const laborExcluded = liabilities.filter(n => under(n, "2.1") && RX.labor.test(n.description) && !isLabor(n) && !n.has_children);
    ...
    const isBorrowing = (n) => RX.borrowings.test(n.description) &&
        !RX.leases.test(n.description) &&
        !RX.finExpenses.test(n.description) &&
        !RX.finRevenues.test(n.description);
    const notBorrowNature = (n) => (RX.tax.test(n.description) || RX.labor.test(n.description) || RX.leases.test(n.description)) && !RX.borrowings.test(n.description);
    ...
    const taxOnProfitNodes = pickNonOverlapping(results, n => RX.incomeTaxes.test(n.description) && !RX.finExpenses.test(n.description));
    ...
    const resultado = Number.isFinite(ctx.resultado) ? ctx.resultado : NaN;
    ...
    const revenueOk = resultCertified
        ...
        && Math.abs(Number.isFinite(ctx.receita_liquida) ? ctx.receita_liquida : receita_base) > 0.01;
...
export function isResultIncluded(mode) {
 ...
 */
export function filterStalePendencias(pendencias, snapshot) {
    if (!pendencias || pendencias.length === 0)
        return [];
    if (!snapshot)
        return pendencias;
    const facts = snapshot.facts;
    const txt = (p) => `${p.title ?? ""} ${p.description ?? ""} ${p.problema ?? ""} ${p.tipo ?? ""} ${p.impacto ?? ""} ${p.recomendacao ?? ""} ${p.fundamentacao ?? ""}`
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
    const revenueOk = Number.isFinite(facts?.receita_liquida) && (facts?.receita_liquida ?? 0) !== 0;
    const inventoryOk = Number.isFinite(facts?.estoques) && (facts?.estoques ?? 0) !== 0;
    const equityOk = Number.isFinite(facts?.patrimonio_liquido) && (facts?.patrimonio_liquido ?? 0) !== 0;
    const resultOk = Number.isFinite(facts?.resultado_liquido) && (facts?.resultado_liquido ?? 0) !== 0;
    return pendencias.filter(p => {
        const t = txt(p);
        // Diagnósticos internos de pipeline nunca são pendências empresariais.
        if (/FATOS DETERMINISTICOS|PIPELINE|EXTRACAO FALHOU|PARSER|PLATFORM|INTERNAL/.test(t))
            return false;
        if (revenueOk && /RECEITA/.test(t) && /(ZERAD|0[.,]00|IGUAL A ZERO|AUSENTE|NAO (FOI )?(EXTRAID|IDENTIFICAD))/.test(t))
            return false;
        if (inventoryOk && /ESTOQUE/.test(t) && /(ZERAD|0[.,]00|IGUAL A ZERO|AUSENTE|NAO (FOI )?(EXTRAID|IDENTIFICAD))/.test(t))
            return false;
        if (equityOk && /PATRIMONIO LIQUIDO/.test(t) && /(ZERAD|AUSENTE|NAO (FOI )?(EXTRAID|IDENTIFICAD))/.test(t))
            return false;
        if (resultOk && /RESULTADO/.test(t) && /(ZERAD|AUSENTE|NAO (FOI )?(EXTRAID|IDENTIFICAD))/.test(t))
            return false;
        // MD-CUTOVER-001 §11: Invalidação de pendências legadas de R$ 17,5M
        if (/PREJUIZOS ACUMULADOS DE R\$ 17,5 MILHOES/i.test(t))
            return false;
        if (t.includes("RECEITA LIQUIDA ZERADA") && revenueOk)
            return false;
        return true;
    });
}
 ...
 */
export function recomputePendencyPercentages(pendencias, ativoTotal) {
    if (!pendencias || pendencias.length === 0)
        return [];
    const at = Number(ativoTotal);
    if (!Number.isFinite(at) || at === 0)
        return pendencias;
    const parseBRL = (raw) => Number(raw.replace(/\./g, "").replace(",", "."));
    const fixField = (text) => {
        if (typeof text !== "string" || !/ATIVO TOTAL/i.test(text))
            return text;
